...
open class SimpleCraft(private val plugin: Plugin, blocks: Collection<Block>, rotationPoint: Location) : MoveableCraft, RotatableCraft, AutoCloseable {
    ...
    open fun containsBlock(block: Block): Boolean {
        if(!boundingBox.contains(block)) {
            ...
            return runBlocking {
                ...
                list.forEach { launch(CommonPool) { channel.send(it.contains(block)) } }
                var answerCount = 0
                channel.consumeEach {
                    if(!it) {
                        answerCount++
                        if(answerCount >= parts) { channel.close(); return@runBlocking false }
                    } else { channel.close(); return@runBlocking true }
                }
    ...
    override fun rotate(rotation: Rotation) {
        ...
        val cannons = ArrayList<Cannon>() // TODO only detect cannons when detecting ship, saves time
